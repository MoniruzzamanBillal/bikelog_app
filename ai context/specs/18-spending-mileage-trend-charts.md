...
**Addendum (2026-07-27, user-reported on-device bug)**: the user reported that on a real device, Mileage's tab bar showed only History/Monthly/Yearly/Lifetime "taking the full width altogether" — the "Trends" tab this spec added was never visible. Root cause: both `Mileage.tsx`'s and `Spending.tsx`'s tab-bar row was a plain `View` with `flexDirection: "row"`, no wrap/scroll — with Mileage's 5 tabs (vs. Spending's 4), the row overflowed the screen width and pushed "Trends" off-screen with no way to reach it. Fixed by wrapping both tab-bar rows in a horizontal `ScrollView` (`showsHorizontalScrollIndicator={false}`, `contentContainerStyle={styles.tabBar}`) — fixed in both files since Spending's identical code shape was a latent instance of the same bug, not yet triggered only because it currently has one fewer tab. No change to `TABS` arrays, tab styling, or content rendering. `expo lint` and `npx tsc --noEmit` both pass clean. **Update, same day, second report**: user confirmed scrolling worked but reported the tab bar now "takes a large height which looks distorted." Root cause: the `ScrollView` had only `contentContainerStyle` set, no `style` of its own — a horizontal `ScrollView` as a flex child in a column-flex parent needs its own cross-axis size bounded, or it won't reliably size to its content's natural height (a known RN gotcha; standard fix is `flexGrow: 0` on the `ScrollView`'s own `style`). Fixed by adding `tabBarScroll: { flexGrow: 0 }` via `style={styles.tabBarScroll}` on both `ScrollView`s, plus `alignItems: "center"` on the `tabBar` style used as `contentContainerStyle`. `expo lint`/`npx tsc --noEmit` both pass clean. **Still not re-confirmed on-device by the user** — ask them to confirm the tab bar now renders at a normal pill height with working horizontal scroll on both Mileage and Spending.
