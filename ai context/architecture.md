...
6. **No global client-state library, no charting library.** Matches both sibling projects' stance. (Spec 18 briefly introduced `react-native-gifted-charts`/`react-native-svg` for the Spending/Mileage trend tabs; spec 25 removed them per direct user instruction, restoring this original stance — see `progress-tracker.md`.)
