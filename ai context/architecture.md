...
6. **No global client-state library.** Matches both sibling projects' stance. Charting is an exception as of spec 18: `react-native-gifted-charts` (+ its peer `react-native-svg`) is used for the Spending/Mileage trend tabs — the "no charting library" stance held for v1 but was deliberately reversed for this v2 feature, mirroring `bikelog_client-web-`'s own spec 13 (Recharts). Don't add a second charting library without a similarly explicit reason.
