...
- **Adapted, not copied verbatim:**
  ...
  - `hooks/useApi.ts` originally dropped `useUpdateData` (a PUT-based hook the reference project has but never really needs either) — `bikelog_server` had no `PUT` routes at the time, same reasoning the web app originally used to drop `apiPut`. **Update (spec 20, image uploads)**: this premise is now stale — `bikelog_server`'s own spec 17 added real `PUT .../image` routes for fuel logs, maintenance logs, and bike accessories, so `apiPut`/`usePut` were added back to both `utils/api.ts`/`hooks/useApi.ts` here (mirroring `usePatch`'s exact shape) and to the web app's equivalent files. Kept this note in place with the correction rather than deleted, since the general point about `useUpdateData` being adapted-not-ported from the reference project still stands.
...
6. **No global client-state library. Charting library: `react-native-gifted-charts` + `react-native-svg`, scoped to the Spending/Mileage trend tabs only.** Spec 18 introduced the charting dependency; spec 25 removed it per direct user instruction (the actual complaint was the donut chart's missing legend, not charts in general); spec 28 restored it with the legend fixed, per direct user instruction — see `progress-tracker.md`. Don't reach for a charting library outside the trend tabs without the same justification.
